...
MINIKUBE_LDFLAGS := -X gitlab.com/hasura/hasuractl-go/pkg/minikube/pkg/version.version=$(VERSION) -X gitlab.com/hasura/hasuractl-go/pkg/minikube/pkg/version.isoVersion=$(ISO_VERSION) -X gitlab.com/hasura/hasuractl-go/pkg/minikube/pkg/version.isoPath=$(ISO_BUCKET)
...
ifeq ($(IN_DOCKER),1)
	CC=o64-clang CXX=o64-clang++ CGO_ENABLED=1 GOARCH=amd64 GOOS=darwin go build --installsuffix cgo -ldflags="$(MINIKUBE_LDFLAGS) $(K8S_VERSION_LDFLAGS)" -a -o $(BUILD_DIR)/minikube-darwin-amd64 gitlab.com/hasura/hasuractl-go/pkg/minikube/cmd/minikube
else
...
out/minikube-linux-amd64: $(GOPATH)/src/$(ORG) pkg/minikube/assets/assets.go $(shell $(MINIKUBEFILES))
	CGO_ENABLED=1 GOARCH=amd64 GOOS=linux go build --installsuffix cgo -ldflags="$(MINIKUBE_LDFLAGS) $(K8S_VERSION_LDFLAGS)" -a -o $(BUILD_DIR)/minikube-linux-amd64 gitlab.com/hasura/hasuractl-go/pkg/minikube/cmd/minikube

out/minikube-windows-amd64.exe: $(GOPATH)/src/$(ORG) pkg/minikube/assets/assets.go $(shell $(MINIKUBEFILES))
	CGO_ENABLED=0 GOARCH=amd64 GOOS=windows go build --installsuffix cgo -ldflags="$(MINIKUBE_LDFLAGS) $(K8S_VERSION_LDFLAGS)" -a -o $(BUILD_DIR)/minikube-windows-amd64.exe gitlab.com/hasura/hasuractl-go/pkg/minikube/cmd/minikube
