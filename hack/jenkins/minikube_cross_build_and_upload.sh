...
GOPATH=$(pwd)/_gopath GOOS=darwin GOARCH=amd64 go test -c github.com/hasura/hasuractl/pkg/minikube/test/integration --tags=integration -o out/e2e-darwin-amd64
GOPATH=$(pwd)/_gopath GOOS=linux GOARCH=amd64 go test -c github.com/hasura/hasuractl/pkg/minikube/test/integration --tags=integration -o out/e2e-linux-amd64
GOPATH=$(pwd)/_gopath GOOS=windows GOARCH=amd64 go test -c github.com/hasura/hasuractl/pkg/minikube/test/integration --tags=integration -o out/e2e-windows-amd64.exe
